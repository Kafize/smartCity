...
<script>
	export default {
		...
		methods: {
			getNewsDetail(){
				this.$request.get('prod-api/api/metro/press/press/'+this.id)
				.then(res=>{
			...
			NewsLike(){
				...
				this.$request.put('prod-api/api/metro/press/press/like/'+this.id)
				.then(res=>{
					if(res.code==200){
			...
			getComment(){
				this.$request.get('prod-api/api/metro/press/comments/list?newsId='+this.id)
				.then(res=>{
					if(res.code==200){
			...
			commonLike(id,index){
				this.$request.put('prod-api/api/metro/press/pressComment/like/'+id)
				.then(res=>{
					if(res.code==200){
			...
			submitCommon(){
				this.$request.post('prod-api/api/metro/press/pressComment',{"content":this.commonContent,'newsId':this.id})
				.then(res=>{
					this.commonContent='';
					if(res.code==200){
						this.$refs.uToast.show({
